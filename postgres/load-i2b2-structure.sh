...
echo >&2 "$(date +"$df") INFO: Initializing i2b2 database based on 'i2b2_data_level' = ${i2b2_data_level:-Not provided}..."
...
su postgres -c "psql -f ${data_dir}/db-schema-user.sql" && \
    su postgres -c "psql -f ${data_dir}/${data_level[${i2b2_data_level}]}" && \
