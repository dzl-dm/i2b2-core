...
sudo -u postgres psql -Ui2b2workdata -qt -c "SELECT COUNT(1) FROM i2b2workdata.WORKPLACE;" i2b2 > /dev/null 2>&1
installed_exitcode=$?
## Passwords updated (Checking it is not the md5sum of "demouser")
# passupdate=$(sudo -u postgres psql -Ui2b2pm -qt -c "SELECT COUNT(1) FROM i2b2pm.pm_user_data where password = '91017d590a69dc49807671a51f10ab7f';" i2b2 | xargs)
