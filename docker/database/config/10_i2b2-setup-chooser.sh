...
## - the administrator deploying the system can select it with "i2b2_initial_data_level"

set -e

df="[%Y-%m-%d %H:%M:%S]"

echo >&2 "$(date +"$df") INFO: Initializing i2b2 database based on 'i2b2_initial_data_level' = ${i2b2_initial_data_level}..."
...
psql -f ${data_dir}/${data_level[${i2b2_initial_data_level}]} && \
