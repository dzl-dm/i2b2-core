...
pg_dumpall --clean | sed -e 's/^DROP ROLE postgres/--&/' -e 's/^CREATE ROLE postgres/--&/' -e 's/^ALTER ROLE postgres/--&/' -e 's/^CREATE DATABASE postgres/--&/' -e 's/^DROP DATABASE postgres/--&/' -e 's/^ALTER DATABASE postgres/--&/' -e 's/^COMMENT ON DATABASE postgres/--&/' > ${data_output_dir}/dumpall-no-project.sql
...
psql -h localhost -Ui2b2workdata -d i2b2 -f ./Workdata/scripts/workplace_access_demo_insert_data.sql
## Hivedata:
cd Hivedata/scripts
# scripts/work_db_lookup_postgresql_insert_data.sql
# - Remove ACT project path
...
psql -h localhost -Ui2b2hive -d i2b2 -f work_db_lookup_postgresql_insert_data_EDIT.sql
...
psql -h localhost -Ui2b2hive -d i2b2 -f ont_db_lookup_postgresql_insert_data_EDIT.sql
...
psql -h localhost -Ui2b2hive -d i2b2 -f crc_db_lookup_postgresql_insert_data_EDIT.sql
# scripts/im_db_lookup_postgresql_insert_data.sql
psql -h localhost -Ui2b2hive -d i2b2 -f im_db_lookup_postgresql_insert_data.sql
cd ../../
## Pmdata:
...
psql -h localhost -Ui2b2pm -d i2b2 -f pm_access_insert_data_EDIT.sql
cd ../../../
## Extract data
echo >&2 "$(date +"$df") INFO: Extracting data after 'create_database' and minimal demo project setup..."
pg_dumpall --clean | sed -e 's/^DROP ROLE postgres/--&/' -e 's/^CREATE ROLE postgres/--&/' -e 's/^ALTER ROLE postgres/--&/' -e 's/^CREATE DATABASE postgres/--&/' -e 's/^DROP DATABASE postgres/--&/' -e 's/^ALTER DATABASE postgres/--&/' -e 's/^COMMENT ON DATABASE postgres/--&/' > ${data_output_dir}/dumpall-demo-empty-project.sql

## Drop i2b2 database and users and restart with whole demo-data...
echo >&2 "$(date +"$df") INFO: Removing all i2b2 data to start a fresh..."
psql -h localhost -Upostgres -d postgres -c "DROP DATABASE i2b2;"
psql -h localhost -Upostgres -d postgres -c "
...
psql -f /docker-entrypoint-initdb.d/02_db-schema-user.sql
cd /tmp/i2b2-data-1.7.13.0002/edu.harvard.i2b2.data/Release_1-7/NewInstall
sleep 0.1
ant -f build.xml create_database
...
pg_dumpall --clean | sed -e 's/^DROP ROLE postgres/--&/' -e 's/^CREATE ROLE postgres/--&/' -e 's/^ALTER ROLE postgres/--&/' -e 's/^CREATE DATABASE postgres/--&/' -e 's/^DROP DATABASE postgres/--&/' -e 's/^ALTER DATABASE postgres/--&/' -e 's/^COMMENT ON DATABASE postgres/--&/' > ${data_output_dir}/dumpall-demo-full-project.sql

## Custom script to remove demo project's data? (~250Mb!)
